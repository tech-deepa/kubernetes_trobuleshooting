...
This is used when we need constrain the manifest creation to the particular node.It allows you to specify a set of key-value pairs that must match the node's labels for a pod to be scheduled on that node.
...
NodeSelector : hardmatch
NodeAffinity: 
1.Required : similar to NodeSelector
2.Prefered try to find the match, if not still schedule the pods.
...
Different types of tains:
1.NoSchedule - when we need to upgrade the node
2.No Execute - when we need to stop all pods running the node
3.PrefferedSchedule - when the performance of the node is reducing
